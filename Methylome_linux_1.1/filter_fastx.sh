...
if [[ $N -eq 2 ]];then
 ...
 ## zcat ${wkdir}/00_data/${PROJECT}/${SRR}_1.fastq.gz ${wkdir}/00_data/${PROJECT}/${SRR}_2.fastq.gz |perl ~/users/wtzhang/script/List_Fasta.pl ${wkdir}/00_data/${PROJECT}/${SRR}_highQsinglereads.list - QF |gzip - > ${wkdir}/00_data/${PROJECT}/${SRR}_se_fastx.gz  &
  ...
  wait
  rm ${wkdir}/00_data/${PROJECT}/${SRR}_fastx.gz &
  wait
  gunzip ${wkdir}/00_data/${PROJECT}/${SRR}_1_fastx.gz &
  gunzip ${wkdir}/00_data/${PROJECT}/${SRR}_2_fastx.gz &
  wait 
  echo "good"
  ...
  gunzip ${wkdir}/00_data/${PROJECT}/${SRR}_fastx.gz &
  wait
  echo "single good"
fi 
cd ${wkdir}
## Remove raw fastq data
FILTERED_SIZE=`du ${wkdir}/00_data/${PROJECT}/${SRR}*_fastx | awk '{print $1}' |head -1`
